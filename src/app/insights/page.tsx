...
import { insightCategories, insights } from "@/lib/freco-content";

export default function InsightsPage() {
  const [active, setActive] = useState("All");
  const [query, setQuery] = useState("");
  const filtered = useMemo(() => insights.filter((insight) => { const matchesCategory = active === "All" || insight.category === active; const matchesQuery = `${insight.title} ${insight.excerpt}`.toLowerCase().includes(query.toLowerCase()); return matchesCategory && matchesQuery; }), [active, query]);
